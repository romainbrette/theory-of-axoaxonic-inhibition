...
In addition, there is a GABAA synapse.
'''
from brian2 import *

__all__ = ['model_Na_Kv1_GABA', 'model_Na_Kv1_GABA_kinetics']


def model_Na_Kv1_GABA(params, resting_vm=75. * mV, Na_start=5. * um, Na_end=35. * um,
    ...
    I_GABA = gGABA*(EGABA-v) : amp (point current)
    gGABA : siemens
    '''
...
def model_Na_Kv1_GABA_kinetics(params, resting_vm=75. * mV, Na_start=5. * um, Na_end=35. * um,
                 gL_soma=1. / (15000. * ohm * cm ** 2),
                 gna_density=4000. * (siemens / meter ** 2), gk_density=1500. * (siemens / meter ** 2), morpho=None,
                 tau_GABA=10*ms):
    '''
    This model has one GABAA synapse with exponential kinetics.
    ...
    neuron = SpatialNeuron(morphology=morpho, model=eqs, Cm=Cm, Ri=Ri,
                           namespace=dict(EL=EL, ENa=ENa, EK=EK,
                                          ...
                                          Kn=Kn, Vn=Vn, Taun_max=Taun_max,
                                          Va_soma=Va_soma,
                                          Vh_soma=Vh_soma, tau_GABA=tau_GABA),
                           method="exponential_euler")
